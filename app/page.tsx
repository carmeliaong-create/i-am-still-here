  ...
  return (
    <main className="desktop" onClick={() => start && setStart(false)} onPointerDown={(e) => { const target = e.target as HTMLElement; startMedia(target); if (target.closest("button, a, input, select")) playSound("key"); }}>
      ...
      <section className="desktop-icons" aria-label="Desktop icons">
        ...
        <a className="desktop-icon external-shortcut" href="https://carmeliaong-create.github.io/the-only-me-is-me/?v=82c6439" target="_self" aria-label="Open The Only Me Is Me" onPointerDown={(event) => { event.preventDefault(); window.location.assign(event.currentTarget.href); }}>
